...
def build(frame, layers=None):
    # Three togglable layers: arch / vortex / dust
    if layers is None: layers = {'arch': True, 'vortex': True, 'dust': True}
    ...
    # Void background (part of arch layer)
    if layers['arch']:
        f(16, 19, 115, 44, 1)
    ...
    if layers['arch']:
        ...
        f(112, 19, 115, 44, 3)
        f(112, 19, 112, 44, 4)
        s(112, 19, 2); s(112, 44, 2)
        s(115, 19, 2); s(115, 44, 2)
        # Rim
        f(14, 17, 15, 46, 5)
        f(14, 17, 115, 18, 5)
        f(14, 45, 115, 46, 5)
    ...
    if layers['dust']:
        # 32 particles with RANDOM lifetimes, distances and angle jitter so
        # they die off at unpredictable times instead of marching in lockstep.
        VCX = 32 + COL_OFF
        VCY = 65 + ROW_OFF
        NUM_PARTICLES = 32
        t = frame / FRAMES
        ...
        for i in range(NUM_PARTICLES):
            start_off = rng(i, 1)
            life = 0.30 + rng(i, 2) * 0.70
            dist_mult = 0.65 + rng(i, 3) * 0.75
            angle_jit = (rng(i, 4) - 0.5) * 0.6
            raw_phase = (t + start_off) % 1
            ...
            while off < -_math.pi: off += TAU
            angle = FORWARD + off * CONE + angle_jit

            dist = (phase ** 0.85) * MAX_DIST * dist_mult
...
def render_pass(name, layer_flags):
    fs = []
    for fr in range(FRAMES):
        g = build(fr, layer_flags)
...
# Build all four passes: full composite + each isolated layer
passes = [
    ('output/magical_portal_animated.gif', {'arch': True,  'vortex': True,  'dust': True }),
    ('output/portal_arch_layer.gif',        {'arch': True,  'vortex': False, 'dust': False}),
    ('output/portal_vortex_layer.gif',      {'arch': False, 'vortex': True,  'dust': False}),
    ('output/portal_dust_layer.gif',        {'arch': False, 'vortex': False, 'dust': True }),
]
for path, flags in passes:
    fs = render_pass(path, flags)
    fs[0].save(
        path,
        save_all=True,
        append_images=fs[1:],
    ...
    print('wrote', path, (W * SCALE, H * SCALE), FRAMES, 'frames')
